use builder_swc::{remove_multiple_ends_code::{RemoveMultipleEndsCode, remove_multiple_ends_code}};
...
fn transform_web_flag_fixture(input: PathBuf) {
  ...
  test_fixture(unminify_syntax(), &|_tr| remove_multiple_ends_code(config.clone()), &input, &output);
}
