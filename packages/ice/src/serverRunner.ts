            ...
            if (options && callback) {
              ...
              if (namespace ? namespace == resolveNamespace : filter && filter.test(resolvedId)) {
                try {
                  ...
                  console.log(err);
                  throw new Error(`Failed to resolve module ${id} from ${importer}, Esbuild name ${plugin.name}: ${err}`);
                }
              }
            }
          }
            ...
            for (const [options, callback] of lifecycles) {
              ...
              if (namespace ? namespace == args.namespace : filter && filter.test(formatedId)) {
                try {
                  ...
                  console.log(err);
                  throw new Error(`Failed to load module ${id}, Esbuild name ${plugin.name}`);
                }
              }
            }
            if (res) {
