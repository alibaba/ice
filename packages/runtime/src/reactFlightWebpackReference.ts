...
let CLIENT_REFERENCE = Symbol.for('react.client.reference');
let PROMISE_PROTOTYPE = Promise.prototype;
let deepProxyHandlers = {
...
const proxyHandlers = {
  get: function (target, name, receiver) {
    switch (name) {
      ...
      case 'then':
        ...
        if (!target.async) {
          ...
          // the client.
          let clientReference = Object.defineProperties({}, {
          ...
          const then = target.then = Object.defineProperties((resolve, reject) => {
            // Expose to React.
    ...
    if (!cachedReference) {
      let reference = Object.defineProperties(() => {
        throw new Error(
...
export function createClientModuleProxy(moduleId: string) {
