...
import type { Compiler, Compilation } from '@ice/bundles/compiled/webpack/index.js';
...
import type { IComponent, IFileType, IMiniFilesConfig } from '../utils/types.js';
...
export default class MiniPlugin {
  ...
  addEntries() {
    ...
    this.pages.forEach((item) => {
      if (item.isNative) {
        ...
        if (item.stylePath) {
          for (const stylePath of item.stylePath) {
            if (fs.existsSync(stylePath)) {
              this.addEntry(stylePath, this.getTargetFilePath(item.name, this.options.fileType.style), META_TYPE.NORMAL);
              break;
            }
          }
        }
  ...
  async generateMiniFiles(compilation: Compilation, compiler: Compiler) {
    ...
    compilation.assets[combination.rawConfig.projectConfigJson] = new RawSource(JSON.stringify(combination.rawConfig.nativeConfig));
