...
import { createRouteId } from '@ice/route-manifest';
...
export class MiniCombination {
  ...
  async readConfig(configPath: string, appPath: string): Promise<any> {
    ...
    if (relativePath === '.ice/entry.miniapp.config') {
    ...
    } else if (relativePath.startsWith('src/pages/') && relativePath.endsWith('.config')) {
      const route = relativePath.slice('src/pages/'.length, -'.config'.length);
      const routeId = createRouteId(`${route}.tsx`);
