...
function getRoutesInfo({ rootDir, tempDir, configPath, projectType, isMpa, srcDir }: IParams): IResult {
  // if is mpa use empty router file
  if (isMpa) {
    const routesTempPath = path.join(tempDir, `routes.${projectType}`);
    fse.writeFileSync(routesTempPath, 'export default [];', 'utf-8');
    return {
      routesPath: routesTempPath,
      isConfigRoutes: true
    };
  }
