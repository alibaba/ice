...
构建 cssModules 时，class 名称只保留 hash 值，以精简 HTML 大小及 CSS 文件大小。

默认情况 `className="custom-head-tab-wrap"` 会被构建为 `class="custom-head-tab-wrap--rAEgGaqM"`，开启 hashOnly 后样式名会被精简为 `class="rAEgGaqM"`。
