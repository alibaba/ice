...
module.exports = async function removePreviewPage({ destDir, isNodeProject }) {
  // 删除 page 和 blocks 文件
  ...
  let routeFilePath = path.join(destDir, clientPath, 'routes.jsx');
  if (!pathExists.sync(routeFilePath)) {
    // hack 兼容 vue 物料 router
    routeFilePath = path.join(destDir, clientPath, 'router.js');
  }
